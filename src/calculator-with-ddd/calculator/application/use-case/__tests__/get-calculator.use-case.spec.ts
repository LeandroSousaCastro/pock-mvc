...
import { CalculatorInMemoryRepository } from '../../../service/db/in-memory/calculator-in-memory-repository';
...
describe('Deve testar o caso de uso de pegar dados da operação matemática realizada', () => {
  ...
  it('deve levantar uma exceção quando não encontrar o registro', async () => {
    expect(() => useCase.execute({ id: '123' })).rejects.toThrow(
      new NotFoundError('Entity Not Found using ID 123'),
    );
  });
